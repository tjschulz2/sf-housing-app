...
const Modal: React.FC<ModalProps> = ({ listing, isOpen, onClose }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(event.target as Node) && !isGalleryOpen) {
        onClose();
      }
    };
    ...
    <Portal zIndex={10}>
      <div className={styles.modalOverlay}>
        <div className={styles.modalContainer} ref={modalRef}>
          ...
          {isSmallScreen && (
            <Carousel images={listing.imageUrls} onImageClick={handleImageClick} />
          )}
          <div className={`flex h-full ${isSmallScreen ? 'w-full' : ''}`}>
            ...
            <div className={`${isSmallScreen ? 'w-full' : 'w-1/2'} h-full overflow-y-auto`}>
              {!isSmallScreen && (
                <header className={`
                  bg-[#FEFBEB]
                  flex items-center
              ...
              <div className="p-6">
                <div className="flex items-baseline mb-2">
                  <div className="font-bold" style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)' }}>{listing.price.toLocaleString('en-US', { style: 'currency', currency: 'USD' }).replace(/\.00$/, '')}</div>
                  <div className="font-bold mr-2" style={{ fontSize: 'clamp(0.875rem, 1.8vw, 1.125rem)' }}>/mo</div>
                  <div className="text-sm text-gray-600 ml-4" style={{ fontSize: 'clamp(0.875rem, 1.8vw, 1.125rem)' }}>
                ...
                <div className="text-sm mb-2 mt-8" style={{ color: '#1D462F', fontSize: 'clamp(0.875rem, 1.8vw, 1.125rem)' }}>
                  ...
                  <div className="flex items-center mt-5">
                    <img src="/images/availablenow.png" alt="Available Now" className="w-[21px] h-[21px] mr-2" />
                    <p style={{ fontSize: 'clamp(0.8rem, 1.6vw, 1rem)', color: '#474747' }}>Available now</p>
                  </div>
                  <div className="flex items-center mt-5">
                    <img src="/images/apartment.png" alt="Apartment" className="w-[21px] h-[21px] mr-2" />
                    <p style={{ fontSize: 'clamp(0.8rem, 1.6vw, 1rem)', color: '#474747' }}>Apartment</p>
                  </div>
                  <div className="flex items-center mt-5">
                    <img src="/images/location.png" alt="Lower Haight" className="w-[18px] h-[21px] mr-2.5" />
                    <p style={{ fontSize: 'clamp(0.8rem, 1.6vw, 1rem)', color: '#474747' }}>Lower Haight</p>
                  </div>
                  <div className="flex items-center mt-5">
                    <img src="/images/deposit.png" alt="Deposit" className="w-[21px] h-[21px] mr-2" />
                    <p style={{ fontSize: 'clamp(0.8rem, 1.6vw, 1rem)', color: '#474747' }}>$3,500 deposit</p>
                  </div>
                </div>
                <div className="text-sm mb-2 mt-8" style={{ color: '#1D462F', fontSize: 'clamp(0.875rem, 1.8vw, 1.125rem)' }}>
